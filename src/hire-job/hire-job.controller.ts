...
export class HireJobController {
  ...
  // Không làm ô nhâp token nên em để userId trên param
  @ApiParam({ name: 'userId', type: Number })
  @Get('/lay-danh-sach-da-thue/:userId')
  async getHiredJob(@Req() req: Request): Promise<ResponseDto> {
    const userId = Number(req.params.userId);
    let checkHiredJob = await this.hireJobService.getHiredJob(userId);
    throw new HttpException(
      {
